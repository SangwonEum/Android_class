	...
	protected void onCreate(Bundle savedInstanceState) {
	 ...
	 private static class ViewWithText extends View {
	        ...
	        public ViewWithText(Context context) {
	            ...
	            mPaint.setTextSize(16);  //글자 크기 설정
	        }
	        
	        @Override protected void onDraw(Canvas canvas) {
	            canvas.drawColor(Color.GREEN);  //배경색 설정

	            mPaint.setTypeface(null);
	            //메소드를 이용해서 원점에서 30,40 픽셀 위치에 글자를 그린다.
	            canvas.drawText("안녕하세요!!! (Normal)", 30, 40, mPaint);

	            mType = Typeface.create(Typeface.SANS_SERIF, Typeface.NORMAL);
	            mPaint.setTypeface(mType); //글꼴 설정
